  ...
  return (
    <>
      <div className="max-w navbar justify-between">
        <div className="">
          <button onClick={() => setOpen(!open)} className="btn btn-ghost text-6xl">
            {
              open && <HiBars3CenterLeft />
            }
          </button>
        </div>
        <div>
      ...
      <div className={`w-[80%] md:w-[40%] lg:w-[25%] h-screen transition-all duration-500 bg-secondary fixed top-0 z-50 ${open ? "-left-96" : 'left-0'}`}>
      ...
      <div className={`w-full h-screen bg-black opacity-70 fixed top-0 z-10 ${open ? 'hidden' : 'visible'}`}></div>
    </>
  );
};
